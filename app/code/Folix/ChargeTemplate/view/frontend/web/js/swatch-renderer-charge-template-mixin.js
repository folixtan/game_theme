...
define([
    ...
    'Folix_ChargeTemplate/js/swatch-dropdown-manager'
], function ($, _, widget, chargeTemplate, htmlBuilder, dropdownManager) {
    'use strict';
    ...
    return function (targetWidget) {
        var chargeTemplateMixin = {
            ...
            _init: function () {
                ...
                if (this._needsCountryDropdown) {
                    dropdownManager.init(this);
                }
            },
            ...
            _RenderSwatchOptions: function (config, controlId) {
                ...
                var isFirstAttribute = (self._renderIndex === 0) && (self._totalAttributes > 1),
                    optionsCount = Object.keys(config.options).length,
                    needsDropdown = isFirstAttribute && optionsCount > 6;
                self._renderIndex++;

                // === Dropdown path (>6 options) ===
                if (needsDropdown) {
                    var pillsHtml = '',
                        selectOptions = '';
                    ...
                    $.each(config.options, function (index) {
                        ...
                        pillsHtml += htmlBuilder.buildPill(optionClass, attr, this.label);
                        selectOptions += '<option value="' + this.id + '" data-option-id="' + this.id + '">' +
                            htmlBuilder.escapeHtml(this.label) + '</option>';
                    });

                    self._needsCountryDropdown = true;
                    self._countryDropdownControlId = controlId;

                    return htmlBuilder.buildDropdownHTML(pillsHtml, selectOptions, config.label);
                }

                // === Regular path (pills / product cards / color / image) ===
                $.each(config.options, function (index) {
                    if (!optionConfig.hasOwnProperty(this.id)) { return; }
            ...
            // ============================================================
            //  5. Rebuild — reset render index, re-init dropdown
            // ============================================================
            _Rebuild: function () {
                this._renderIndex = 0;
                this._needsCountryDropdown = false;
                this._super();
